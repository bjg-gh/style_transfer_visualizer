...
def test_csv_file_creation(tmp_path: Path):
...
def test_log_writes_rows(tmp_path: Path):
    ...
    with csv_path.open("r", newline="") as f:
        ...
        assert reader[0] == ["step", "style_loss", "content_loss", "total_loss"]
...
def test_close_closes_file(tmp_path: Path):
...
def test_invalid_path_raises(tmp_path: Path, mocker):
...
def test_flush_called_on_log(tmp_path: Path):
...
@pytest.mark.parametrize(
    "file_value, closed_value",
    [
        (None, None),             # Simulate self.file = None
        ("mock_file", True),      # Simulate self.file.closed == True
    ]
)
def test_close_noop_when_file_missing_or_closed(tmp_path: Path, file_value, closed_value):
    """Test that close() does nothing if file is None or already closed."""
    logger = LossCSVLogger(tmp_path / "losses.csv", log_every=1)
    if file_value is None:
        logger.file = None
    else:
        logger.file.close()  # Close file to simulate already closed
    # Should not raise
    logger.close()