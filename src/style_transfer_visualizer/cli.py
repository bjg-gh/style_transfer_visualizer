...
from style_transfer_visualizer.config_defaults import DEFAULT_ADAM_LR
...
Note:
  Normalization is enabled by default. Use --no-normalize to disable it"""
    ...
    opt.add_argument(
        ...
        default=argparse.SUPPRESS)
    opt.add_argument(
        "--optimizer", choices=["lbfgs", "adam"],
        help="Optimizer to use", default=argparse.SUPPRESS)
    opt.add_argument(
...
def log_parameters(p: dict, args: argparse.Namespace) -> None:
    ...
    logger.info("Optimizer: %s", p["optimizer_name"])
...
def run_from_args(args: argparse.Namespace):
    ...
    params = {
        ...
        "optimizer_name": get("optimizer", "optimization"),
...
def main() -> None:
    ...
    if hasattr(args, "optimizer"):
        if args.optimizer == "adam" and not hasattr(args, "lr"):
            args.lr = DEFAULT_ADAM_LR
